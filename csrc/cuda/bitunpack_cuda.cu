    ...
    int64_t kernel = 0)
{
    ...
    auto [P, K] = check_inputs(bitpacked, N, M);

    // Instantiate the output, make sure to zero pad so that it has an element number which is a
    // multiple of the writes per block
    const uint32_t writes_per_block = THREADS_PER_BLOCK * 8;
    torch::Tensor unpacked;
    if ((P * 8 * K) % writes_per_block == 0)
    {
        unpacked = torch::zeros({P * 8 * K}, torch::TensorOptions().dtype(torch::kBool).device(bitpacked.device())); // [P * 8K]
    }
    else
    {
        const uint32_t padding = writes_per_block - ((P * 8 * K) % writes_per_block);
        unpacked = torch::zeros({P * 8 * K + padding}, torch::TensorOptions().dtype(torch::kBool).device(bitpacked.device())); // [P * 8K + pad]
    }
    ...
    auto bitpacked_ptr = bitpacked.data_ptr<uint8_t>();
    auto unpacked_ptr = unpacked.data_ptr<bool>();

    if (kernel == 0)
    {
        unpack_kernel<<<blocks, threads>>>(bitpacked_ptr, unpacked_ptr);
    }
    else
    {
        naive_unpack_kernel<<<blocks, threads>>>(bitpacked_ptr, unpacked_ptr);
    }
