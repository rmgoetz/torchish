...
def binmatmul_2d(A: torch.Tensor, B: torch.Tensor) -> torch.Tensor:
    """Perform matrix multiplication with bitpacked boolean tensors.

    Args:
        A (torch.Tensor): A [N, K] bitpacked tensor.
        B (torch.Tensor): A [K, M] bitpacked tensor.

    Returns:
        torch.Tensor: The [N, M] bitpacked tensor product of matrix multiplying
                      A on the right by B.
    """
